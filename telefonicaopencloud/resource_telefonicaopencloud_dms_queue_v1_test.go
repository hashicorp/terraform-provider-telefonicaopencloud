		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckDmsV1QueueExists("telefonicaopencloud_dms_queue_v1.queue_1", queue),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_dms_queue_v1.queue_1", "name", queueName),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_dms_queue_v1.queue_1", "queue_mode", "NORMAL"),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_dms_queue_v1.queue_1", "name", queueName),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_dms_queue_v1.queue_1", "description", "test create dms queue"),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_dms_queue_v1.queue_1", "queue_mode", "FIFO"),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_dms_queue_v1.queue_1", "redrive_policy", "enable"),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_dms_queue_v1.queue_1", "max_consume_count", "80"),
				),
			},
		},
	})
...
func testAccCheckDmsV1QueueDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefonicaopencloud_dms_queue_v1" {
			continue
		}
