		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckVBSBackupV2Exists("telefonicaopencloud_vbs_backup_v2.backup_1", &config),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_vbs_backup_v2.backup_1", "name", "vbs-backup"),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_vbs_backup_v2.backup_1", "description", "Backup_Demo"),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_vbs_backup_v2.backup_1", "status", "available"),
				),
			},
		},
	})
...
func testAccCheckVBSBackupV2Destroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefonicaopencloud_vbs_backup_v2" {
			continue
		}
