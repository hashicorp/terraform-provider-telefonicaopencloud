		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCSBSBackupV1Exists("telefonicaopencloud_csbs_backup_v1.csbs", &backups),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_csbs_backup_v1.csbs", "backup_name", "csbs-test1"),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_csbs_backup_v1.csbs", "resource_type", "OS::Nova::Server"),
				),
			},
		},
	})
...
func testAccCSBSBackupV1Destroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefonicaopencloud_csbs_backup_v1" {
			continue
		}
