		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccVBSBackupPolicyV2Exists("telefonicaopencloud_vbs_backup_policy_v2.vbs", &policy),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_vbs_backup_policy_v2.vbs", "name", "policy_001"),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_vbs_backup_policy_v2.vbs", "status", "ON"),
				),
			},
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccVBSBackupPolicyV2Exists("telefonicaopencloud_vbs_backup_policy_v2.vbs", &policy),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_vbs_backup_policy_v2.vbs", "name", "policy_002"),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_vbs_backup_policy_v2.vbs", "status", "ON"),
				),
			},
...
func testAccVBSBackupPolicyV2Destroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefonicaopencloud_vbs_backup_policy_v2" {
			continue
		}
