		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckVpcV1Exists("telefonicaopencloud_vpc_v1.vpc_1", &vpc),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_vpc_v1.vpc_1", "name", "terraform_provider_test"),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_vpc_v1.vpc_1", "cidr", "192.168.0.0/16"),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_vpc_v1.vpc_1", "status", "OK"),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_vpc_v1.vpc_1", "shared", "false"),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckVpcV1Exists("telefonicaopencloud_vpc_v1.vpc_1", &vpc),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_vpc_v1.vpc_1", "name", "terraform_provider_test1"),
				),
			},
		},
	})
...
func testAccCheckVpcV1Destroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefonicaopencloud_vpc_v1" {
			continue
		}
