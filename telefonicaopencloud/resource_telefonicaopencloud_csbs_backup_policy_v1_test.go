		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCSBSBackupPolicyV1Exists("telefonicaopencloud_csbs_backup_policy_v1.backup_policy_v1", &policy),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_csbs_backup_policy_v1.backup_policy_v1", "name", "backup-policy"),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_csbs_backup_policy_v1.backup_policy_v1", "status", "suspended"),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCSBSBackupPolicyV1Exists("telefonicaopencloud_csbs_backup_policy_v1.backup_policy_v1", &policy),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_csbs_backup_policy_v1.backup_policy_v1", "name", "backup-policy-update"),
				),
			},
		},
	})
...
func testAccCheckCSBSBackupPolicyV1Destroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefonicaopencloud_csbs_backup_policy_v1" {
			continue
		}
...
resource "telefonicaopencloud_csbs_backup_policy_v1" "backup_policy_v1" {
	name            = "backup-policy"
  	resource {
      id = "${telefonicaopencloud_compute_instance_v2.instance_1.id}"
      type = "OS::Nova::Server"
      name = "resource4"
  	}
  	scheduled_operation {
