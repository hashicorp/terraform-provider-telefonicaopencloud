		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckRTSStackV1Exists("telefonicaopencloud_rts_stack_v1.stack_1", &stacks),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_rts_stack_v1.stack_1", "name", "terraform_provider_stack"),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_rts_stack_v1.stack_1", "status", "CREATE_COMPLETE"),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_rts_stack_v1.stack_1", "disable_rollback", "true"),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_rts_stack_v1.stack_1", "timeout_mins", "60"),
				),
			},
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckRTSStackV1Exists("telefonicaopencloud_rts_stack_v1.stack_1", &stacks),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_rts_stack_v1.stack_1", "disable_rollback", "false"),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_rts_stack_v1.stack_1", "timeout_mins", "50"),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_rts_stack_v1.stack_1", "status", "UPDATE_COMPLETE"),
				),
			},
		},
	})
...
func testAccCheckRTSStackV1Destroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefonicaopencloud_rts_stack_v1" {
			continue
		}
